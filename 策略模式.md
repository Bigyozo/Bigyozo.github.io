# 策略模式
### 基于策略模式，用EditRuleFactoryUtil替换原有RuleHandler中的if-else逻辑，实现了对编辑后事件业务代码的解耦。
在规则处理Rule类上添加@EditRule注解并标注编辑后事件对应字段名
```
package nccloud.pubitf.dm.pub.utils;
