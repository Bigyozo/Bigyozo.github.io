...
### 基于注解及包文件扫描实现的策略模式，主要用于对 if-else 逻辑进行解耦。

代码中用 EditRuleFactoryUtil 替换原有 RuleHandler 中重复使用的 new 处理类 if-else 逻辑，往后添加新逻辑时只需要在原包路径下添加带注解的处理类(IHeadAfterRule)，无需修改 RuleHandler 类,从而实现了对编辑后事件业务代码的解耦。

在规则处理 Rule 类上添加@EditRule 注解并标注编辑后事件对应字段名
...
生成 Rule 的工厂，传入包名和字段返回规则处理 Rule 类
